...
# Import Data
districts_burglary_shp <- readOGR(dsn = "C:/Users/91875/Desktop/LA Crime Spatial AC Project/LAPD_Reporting_Districts.shp")
...
head(districts_burglary_shp@data)
length(unique(districts_burglary_shp@data$REPDIST))
# So, we have 1135 unique dist codes in the shape file

# taking a look at the crime data
head(crime_data)

#Our objective is to find out if there are spatial clustering for Burglary crime numbers across police districts    
#we need to sum up all the burg crimes against women on a district level
burglary_only <- subset(crime_data,Crm.Cd.Desc == "BURGLARY"|Crm.Cd.Desc == "BURGLARY, ATTEMPTED"|Crm.Cd.Desc == "BURGLARY FROM VEHICLE")
nrow(burglary_only)
w1<-table(burglary_only$Rpt.Dist.No)
head(w1)

#Putting the district codes to district_burg data frame
district_burg<-as.data.frame(w1) 
head(district_burg)

#Looking at the no of uniq districts in the burg data
length(unique(district_burg$Var1))

# There are 1138 distinct districts.Now there is a problem. The no of districts
...
#Now merging two data with burglary_only$Rpt.Dist.No and districts@data$REPDIST
districts_burglary_shp@data <- merge(districts_burglary_shp@data,district_burg, by.x = "REPDIST", by.y = "Var1", all.x = TRUE)

#replacing NA with 0
districts_burglary_shp$Freq[is.na(districts_burglary_shp$Freq)] <- 0
       
head(districts_burglary_shp@data)

#Plotting crime frequency
var <- districts_burglary_shp@data[,"Freq"]
head(var)
...
#plotting crime freq
plot(districts_burglary_shp, col = my_colours[findInterval(var, breaks$brks, all.inside = TRUE)],   
     axes = FALSE, border = NA)
legend(x = -118.7, y = 34, legend = leglabs(breaks$brks), fill = my_colours, bty = "n", cex = 0.6)

#We can see some isolated instances of high burglary numbers in the north and south parts of LA
...
nb <- poly2nb(districts_burglary_shp, queen=TRUE)
...
MC<- moran.mc(districts_burglary_shp$Freq, lw, nsim=999)
MC
plot(MC, main="", las=1)

#The Global Moran I statistic is 0.067062 and p value is 0.001 (positive and significant), showing that there is some clustering of like values.
...
moran <- moran.plot(districts_burglary_shp$Freq, listw = nb2listw(nb, style = "W"))

#Compute Local Moran
?localmoran
local <- localmoran(x = districts_burglary_shp$Freq, listw = nb2listw(nb, style = "W"))
head(local)
...
moran.map <- cbind(districts_burglary_shp, local)
...
m.crimes <- districts_burglary_shp$Freq - mean(districts_burglary_shp$Freq)     
...
plot(districts_burglary_shp,border="lightgray",col=colors[findInterval(quadrant,brks,all.inside=FALSE)])
box()
